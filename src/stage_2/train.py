...
def main():
    ...
    tokenizer.add_tokens([f'<xRAG>'])
    
    model: ReconstructModel = ReconstructModel.build(args=args, tokenizer=tokenizer)
    logger.info(model)
    ...
    data_loader = PretrainingDataLoader(args=args, tokenizer=tokenizer, ret_tokenizer=ret_tokenizer)
    ...
    if args.do_train:
        ...
        trainer.save_model()
        pretrained_backbone = model.llm.merge_and_unload()
        pretrained_backbone.save_pretrained(os.path.join(args.output_dir))
